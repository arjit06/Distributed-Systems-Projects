...
def search_item(stub):
    ...
    category = input(
        "Enter item category (ELECTRONICS, FASHION, OTHERS, ANY): ").upper()

    # print(f"Market prints: Search request for Item name: {item_name if item_name else '<empty>'}, Category: {category}.")

    response = stub.SearchItem(generate_pb2.SearchItemRequest(
...
def buy_item(stub):
    ...
    response = stub.BuyItem(generate_pb2.BuyItemRequest(
...
def add_to_wishlist(stub):
    ...
    response = stub.AddToWishList(generate_pb2.AddToWishListRequest(
...
def rate_item(stub):
    ...
    response = stub.RateItem(generate_pb2.RateItemRequest(
...
def HandleBuyerNotifications(request):
    ...
    print(
        f"Seller with address {seller_address} has updated the price of item with id {item_id} to ${new_price} and quantity to {new_quantity}")
